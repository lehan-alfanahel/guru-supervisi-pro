      ...
      <div className="p-4 space-y-4 max-w-lg mx-auto">
        ...
        <Tabs defaultValue="administrasi">
          <TabsList className="w-full grid grid-cols-5">
            ...
            <TabsTrigger value="hasil_adm" className="text-[10px] px-1">
              Hasil Adm.
              {supervisions.length > 0 && (
                <Badge className="ml-0.5 bg-blue-600/20 text-blue-700 border-0 text-[9px] px-1">{supervisions.length}</Badge>
              )}
            </TabsTrigger>
          ...
          {/* ── TAB HASIL SUPERVISI ADMINISTRASI ── */}
          <TabsContent value="hasil_adm" className="space-y-4 mt-4">
            <div>
              <h2 className="text-base font-bold">Hasil Supervisi Administrasi</h2>
              <p className="text-xs text-muted-foreground">Penilaian dari kepala sekolah — {supervisions.length} penilaian</p>
            </div>
            {supervisions.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                  <ClipboardList className="w-16 h-16 text-muted-foreground mb-4" />
                  <p className="text-lg font-semibold mb-1">Belum ada hasil penilaian</p>
                  <p className="text-sm text-muted-foreground">Data muncul setelah kepala sekolah melakukan penilaian supervisi administrasi</p>
                </CardContent>
              </Card>
            ) : (
              <div className="space-y-3">
                {supervisions.map((s: any, index: number) => {
                  const score = calculateSupScore(s);
                  const pct = Math.round((score / SCORE_MAX) * 100);
                  const predikat = getPredikat(pct);
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between gap-2 mb-3">
                          <div>
                            ...
                            {s.mata_pelajaran && (
                              <p className="text-xs font-medium mt-1">{s.mata_pelajaran}</p>
                            )}
                          </div>
                          <div className="flex gap-1 flex-shrink-0">
                        ...
                        {isExpanded && (
                          <div className="mt-3 border-t pt-3 space-y-1.5">
                            {SUPERVISION_COMPONENTS.map((comp) => {
                              const v = Number(s[comp.key]) || 0;
                              const badge = [
                                { text: "Tidak Ada", cls: "text-destructive" },
                                { text: "Ada tapi tidak sesuai", cls: "text-yellow-600" },
                                { text: "Ada dan sesuai", cls: "text-green-600" },
                              ][v];
                              return (
                                <div key={comp.key} className="flex items-start justify-between gap-2 text-xs py-0.5">
                                  <span className="text-muted-foreground flex-1">{comp.label}</span>
